...
public class SVG
{
   ...
   public void renderToBitmap(Context context, Bitmap bitmap) {
      RenderOptions renderOptions = new RenderOptions(context);

      if (!renderOptions.hasViewPort()) {
         renderOptions.viewPort(0f, 0f, (float) bitmap.getWidth(), (float) bitmap.getHeight());
      }

      SVGAndroidRenderer  renderer = new SVGAndroidRenderer(bitmap, this.renderDPI);
   ...
   */
   ...
   static class Filter extends Group {
     @Override String getNodeName() {
       return "filter";
     }
   }

   static class FeGaussianBlur extends SvgElementBase {
      String in;
      float stdDeviation;
