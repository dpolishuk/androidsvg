...
class SVGParser
{
   ...
   {
      Log.d("ddd", "parseUsingXmlPullParser");
...
Log.d(TAG,"PROC INSTR: "+parser.getText());
   ...
   private void startElement(String uri, String localName, String qName, Attributes attributes) throws SVGParseException
   {
      ...
      switch (elem)
      {
        ...
        case filter:
            filter(attributes); break;
         case feGaussianBlur:
            feGaussianBlur(attributes); break;
         case stop:
  ...
  private void  text(String characters) throws SVGParseException
   ...
   private void  parseAttributesSVG(SVG.Svg obj, Attributes attributes) throws SVGParseException
   {
    ...
    private void filter(Attributes attributes) throws SVGParseException {
      debug("<filter>");

      if (currentElement == null)
        throw new SVGParseException("Invalid document. Root element must be <svg>");
      SVG.Group  obj = new SVG.Filter();
      obj.document = svgDocument;
      obj.parent = currentElement;
      parseAttributesCore(obj, attributes);
      parseAttributesStyle(obj, attributes);
  ...
  private void feGaussianBlur(Attributes attributes) throws SVGParseException
  {
    ...
    SVG.FeGaussianBlur  obj = new SVG.FeGaussianBlur();
    obj.document = svgDocument;
    obj.parent = currentElement;
    parseAttributesCore(obj, attributes);
    parseAttributesStyle(obj, attributes);
    parseAttributesFeGaussianBlur(obj, attributes);
    currentElement.addChild(obj);
  }

   private void  parseAttributesFeGaussianBlur(SVG.FeGaussianBlur obj, Attributes attributes) throws SVGParseException
   {
      for (int i=0; i<attributes.getLength(); i++)
      {
         String val = attributes.getValue(i).trim();
         ...
         {
            case stdDeviation:
               obj.stdDeviation = parseFloat(val);
               break;
            case in:
               obj.in = val;
               break;
   ...
   private void  parseAttributesCore(SvgElementBase obj, Attributes attributes) throws SVGParseException
   {
      for (int i=0; i<attributes.getLength(); i++)
      {
         ...
         if (qname.equals("id") || qname.equals("xml:id"))
         {
            ...
            String filterId = attributes.getValue(i).trim();

            List<SvgObject> svgObjects = obj.parent.getChildren();

            for (int j = 0; j < svgObjects.size(); ++j) {
               if (svgObjects.get(j) instanceof SVG.Filter) {
                  if (filterId.equals(((SvgElementBase) svgObjects.get(j)).id)) {
                     obj.filter = (SVG.Filter) svgObjects.get(j);
                  }
               }
            }
         }
